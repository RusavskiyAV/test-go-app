...
func (h *Handler) Handle(c Command, createHandler accountUseCaseCreate.Interface) error {
	_ = createHandler.Handle(accountUseCaseCreate.Command{UserId: c.ReceiverId})
	...
	err = h.AccountRepository.TransferByUserId(c.UserId, c.ReceiverId, money, time.Now().UTC())
